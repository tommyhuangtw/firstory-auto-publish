...
- **務必**在文章中插入 **1–2 張**圖片（必要、不可省略），且你要當「美術指導」，依該段內容判斷用哪種圖，在最適合的 section 之間「獨立一行」放標記：
  - **首選、預設**：品牌編輯插畫，格式 \`[[SLOTH: 中文brief]]\`。brief 要「先描述這段要視覺化的『概念／隱喻』本身（這是畫面主體）」，再補一句湯懶懶可以怎麼當『配角點綴』融入（比例小、不搶戲）。專業調性、**不要**耍廢元素。**第一張圖一律用 SLOTH**。例：\`[[SLOTH: 一條本來平緩的折線突然像火箭往上衝、旁邊散落自動生成的程式碼方塊，隱喻 AI 讓程式碼產出暴增；湯懶懶在畫面一角從容地看著]]\`
