...
export async function regenerateIgCaption(
  ...
  const episodeInfo = episodeTitle
    ? `本集資訊：${episodeTitle}${episodeDate ? `（${episodeDate}）` : ''}`
    : '';
...
🎯 任務目標
根據提供的 Podcast 內容摘要，生成一則以核心 insights 為主軸的貼文，適合吸引機器人與 AI 愛好者在 IG 上閱讀、收藏與互動。
...
2. 🎥 今日機器人重點整理（3-5 個核心 insight，禁止加入湯懶懶 murmur 或評語）
請以 emoji 開頭，每個 insight 包含：
一行：insight 標題（中文，可以是機器人產品、技術突破、產業觀點或趨勢——根據內容決定）
兩行：核心內容、為什麼值得關注（注意要用中文！！！）
...
🎯 任務目標
根據提供的 Podcast 內容摘要，生成一則以核心 insights 為主軸的貼文，適合吸引 AI 愛好者在 IG 上閱讀、收藏與互動。
...
在開場 murmur 之後、進入重點摘要之前，用一行標示單元名稱。
若有提供本集資訊（EP 集數、標題、日期），必須使用完整標題，禁止截短或改寫。格式：
「${isWeekly ? '📋 AI懶人精選週報' : '📰 AI懶人報'}｜{完整標題}（{日期}）」
此行獨立存在，不與其他段落合併。

2. 🎥 今日重點整理（3-5 個核心 insight，禁止加入湯懶懶 murmur 或評語）
請以 emoji 開頭，每個 insight 包含：
一行：insight 標題（中文，可以是工具名、觀點、方法或趨勢——不要硬湊工具，根據內容決定）
兩行：核心內容、為什麼值得關注（注意要用中文！！！）
✅ 重點應簡單、明確、有條理、每個重點之間要有空行
⛔ 禁止使用湯懶懶口吻、廢話、轉場語或任何非資訊性內容
...
🎯 任務目標：
請根據輸入的 JSON 格式內容（包含 ${videoCount} 部影片的摘要資料），撰寫一段結構清晰、有趣易讀的繁體中文週報內容，統整過去一週內最具話題性的機器人產業與技術更新影片。

**⚠️ 請務必注意：即使影片主題相近，也「不可合併」、「不可刪除」任何一部影片，**請依據提供的 ${videoCount} 筆資料，完整輸出 ${videoCount} 條影片摘要。**⚠️ 嚴禁自行新增、虛構或捏造任何影片、YouTube 連結、觀看數或互動數據——只能使用上方實際提供的影片資料，提供幾筆就輸出幾筆。**
...
🎯 任務目標：
請根據輸入的 JSON 格式內容（包含 ${videoCount} 部影片的摘要資料），撰寫一段結構清晰、有趣易讀的繁體中文週報內容，統整過去一週內最具實用性與話題性的 AI 工具影片。
...
🎯 任務目標：
請根據輸入的 JSON 格式內容（包含 ${videoCount} 部影片的摘要資料），撰寫一段結構清晰、有趣易讀的繁體中文週報內容，統整過去三週內最具實用性與話題性的 AI 工具影片。
