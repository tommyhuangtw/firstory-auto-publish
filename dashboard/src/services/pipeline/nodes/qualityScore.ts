...
1. 聊天感與語氣自然度（25 分）
   ...
   ❌ 扣分指標：長句過多或語法複雜，如報告式、論文式敘述；重複使用單一口氣（如全篇都是「這個工具可以...」「這個工具也可以...」）；沒有情緒語助詞，全篇聽感平淡；語助詞使用位置不合理（如用在資訊量重、理性說明的段落）；句尾語助詞機械性過度使用（「啦」「喔」「啊」「嘛」），導致語感拖沓、尷尬、不真實；全篇像中性資訊播報，對每個主題的態度都一樣；沒有任何個人反應或觀點；整篇零共鳴感，聽眾找不到自己的影子；每句長度都差不多、唸起來像在報數（缺乏長短句交錯的節奏）
   ❌ AI 文體重扣：出現以下任一項都應在此維度扣 3-5 分：使用「不是…而是」「不只…還是」「不只是…而是」「不需要…只需要」等對比句式、「接住」「對齊」的比喻用法、「有點灰」「這筆帳」「書籤起來」（應為「收藏起來」）、「讓人癱瘓」（應為「讓人瘋掉」）、「把…抽乾」（應為「耗盡」「消磨」）、「更大的背景」（應為「把背景交代清楚」）、「無痛升級」「被打到」「很貼」「降維打擊」「定心丸」、不自然的比喻（如「當成搖搖球在用」）；過度縮減詞語（如「跳步驟」→「跳過步驟」、「很穩」→「很穩定」、「撐得住」→「支撐得住」）；過度使用「老實說」（整篇最多一次，可用「說真的」替代）；結構型 AI 痕跡——句尾掛空泛分詞評論（「象徵著…」「凸顯了…」「反映出更大的…」）、硬湊三項排比（剛好三項、節奏一致的列舉）、空泛掛名（「專家認為」「業界普遍…」這種講不出來源的）、假範圍「從 X 到 Y」、重要性灌水（「標誌著轉捩點」「見證了」）——這些都是 AI 生成文體的典型特徵，會讓聽眾覺得不自然
...
❌ 結構型 AI 痕跡：句尾掛空泛分詞評論（「象徵著…」「凸顯了…」「反映出更大的…」）、硬湊三項排比（剛好三項、節奏一致的列舉）、空泛掛名（「專家認為」「業界普遍…」）、假範圍「從 X 到 Y」、重要性灌水（「標誌著轉捩點」「見證了」）→ 直接講事實。
✅ 節奏：句子長短交錯，別每句一樣長（唸起來才不像報數）。但短句仍要完整好唸，不可為了短而剁斷句子——「不要過度簡化、詞語要完整」優先，節奏靠長短交錯達成，不是靠把句子壓短。
❌ 禁止出現以下內容：
