...
const CHARACTER = `「湯懶懶 Mr. Sloth」是 AI 懶人報的品牌角色——一隻聰明、從容、專業，但同時可愛、有親和力的樹懶。請嚴格參考提供的參考圖，保持同一個角色的外型長相；要「專業但可愛」：保留討喜的表情與溫度，但姿態自信、專業，不死板嚴肅、也不耍廢。`;

const COMPOSITION = `構圖重點（很重要）：這張圖是用來「支持文章裡的某個洞見或數據」，不是裝飾。畫面「主體」就是那個要凸顯的洞見／數據本身，把它清楚地視覺化（例如圖表、前後對比、關鍵大數字、機制示意），讓讀者一眼看懂重點——要像一張「會說話的編輯插圖」。湯懶懶只是帶品牌風格的「小配角」，在一旁引導或反應、比例不用大、不要佔滿畫面、不一定在正中央。重點是那個洞見／數據，不是樹懶。`;
