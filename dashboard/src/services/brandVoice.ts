...
## 結構型 AI 痕跡（跟用詞無關，句子怎麼搭的毛病）
- 句尾掛空泛分詞評論（「象徵著…」「凸顯了它的重要性」「反映出更大的趨勢」）→ 講完就停，別補一句幫它下意義的尾巴
- 硬湊三項排比（「又快又穩又便宜」這種剛好三拍的列舉）→ 改成兩項或四項
- 空泛掛名（「專家認為」「業界普遍…」這種講不出來源的）→ 點出具體來源，或整句拿掉
- 假範圍「從 X 到 Y」（X、Y 根本不同尺度硬湊涵蓋感）、重要性灌水（「標誌著轉捩點」「見證了」）→ 拿掉，只講實際發生的事`;
...
export const BANNED_PATTERNS: { label: string; re: RegExp }[] = [
  ...
  { label: '空泛分詞評論尾巴', re: /[，,][^。！？\n]{0,12}(象徵著|凸顯了|彰顯了|體現了|見證了|反映了更)/ },
  { label: 'AI買詞', re: /(賦能|底層邏輯|頂層設計|抓手|閉環|賽道|風口|降維|護城河|長期主義|核心競爭力|儀式感|無縫|一站式|全方位|生態系|種草|破圈|觸達|鏈路|卡位|複利|本質|到位|迭代|對齊|All-?in|革命性|顛覆|game ?changer|next ?level|深度解析|一文看懂|乾貨滿滿|看完秒懂|超到位|深刻)/i },
];
