...
// Reply-zone themes Tommy wants: AI / 創業一人公司 / 接案 / 職涯 / 留學 / 英國生活 / 美國生活.
...
const NICHE_BUSINESS = /(創業|一人公司|個人品牌|獨立開發|獨立接案|自媒體|副業|side ?project|solo ?(founder|preneur)|indie ?hacker|bootstrap|接單|開公司|做產品|營收|訂閱制)/i;
...
export function isNicheRelevant(text: string): boolean {
  ...
  if (NICHE_BUSINESS.test(text) || NICHE_FREELANCE.test(text) || NICHE_CAREER.test(text) || NICHE_STUDY.test(text)) return true;
