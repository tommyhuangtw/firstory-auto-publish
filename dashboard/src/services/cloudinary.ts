 ...
 * Returns the public secure_url for use with Instagram Reels.
 */
export async function uploadVideoToCloudinary(source: string | Buffer, filename?: string): Promise<string> {
  const cloudName = process.env.CLOUDINARY_CLOUD_NAME;
  const preset = process.env.CLOUDINARY_UPLOAD_PRESET;
  if (!cloudName || !preset) throw new Error('CLOUDINARY_CLOUD_NAME and CLOUDINARY_UPLOAD_PRESET required');
  ...
  if (!resp.ok) {
    const text = await resp.text();
    throw new Error(`Cloudinary video upload failed: ${resp.status} ${text}`);
  }

  const data = await resp.json();
  log.info({ url: data.secure_url }, 'Uploaded video to Cloudinary');
  return data.secure_url;
}
