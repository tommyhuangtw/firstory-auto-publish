...
export function TaskDrawer({ task, onClose, onTaskUpdated }: TaskDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className={`fixed top-0 right-0 h-full w-full max-w-xl bg-zinc-950 border-l border-zinc-800 z-50
      ...
      `}>
        {task && (
          <>
            {/* Drawer header */}
            <div className="flex items-start gap-3 px-5 py-4 border-b border-zinc-800/60">
              <div className="flex-1 min-w-0">
                ...
                {task.description && (
                  <p className="text-xs text-zinc-500 mt-1 leading-relaxed line-clamp-3">{task.description}</p>
                )}
              </div>
              <button onClick={onClose}
                className="shrink-0 p-1.5 rounded-lg hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300 transition-colors">
                ✕
              </button>
            </div>
            ...
            {/* Result notes (if any) */}
            {task.result_notes && (
              <div className="mx-5 mt-3 p-3 rounded-lg bg-green-500/8 border border-green-500/20">
