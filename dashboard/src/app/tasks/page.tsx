...
function TaskCard({ task, onUpdate, isDragging = false }: {
  task: Task;
  onUpdate: () => void;
  isDragging?: boolean;
}) {
...
function DragOverlayCard({ task }: { task: Task }) {
  return <TaskCard task={task} onUpdate={() => {}} isDragging />;
}

// ─── Column ───────────────────────────────────────────────────────────────────

function KanbanColumn({
  col, tasks, onUpdate, isOver,
}: {
...
function NewTaskModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const submit = async () => {
    ...
    await fetch('/api/tasks', {
      ...
      body: JSON.stringify({ ...form, auto_execute: form.auto_execute ? 1 : 0, scheduled_at: form.scheduled_at || null, created_by: 'manual' }),
    });
    setLoading(false);
    onCreated();
    onClose();
      ...
      <div className="bg-zinc-900 border border-zinc-700/60 rounded-xl w-full max-w-md p-5 space-y-4 shadow-2xl" onClick={e => e.stopPropagation()}>
        ...
        <div>
          ...
          <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
            rows={3} className="w-full bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-2 text-sm text-zinc-100 focus:outline-none focus:border-zinc-500 resize-none placeholder-zinc-600"
            placeholder="詳細說明..." />
        </div>
        ...
        <label className="flex items-center gap-2.5 cursor-pointer select-none">
          <input type="checkbox" checked={form.auto_execute} onChange={e => setForm(f => ({ ...f, auto_execute: e.target.checked }))}
