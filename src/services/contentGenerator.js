...
class ContentGenerator {
  ...
  // 手動上傳用：直接接受純文字
  async generateFromText(plainText, segment = null) {
    const textContent = plainText.substring(0, 4000); // 限制長度
    return this._generateTitleAndDescription(textContent, segment);
  }

  // Airtable 流程用：從 Email HTML 提取文字後生成
  async generateFromEmailHtml(emailHtml, segment = null) {
    const textContent = this.extractTextFromHtml(emailHtml);
    console.log(`📝 提取的文字內容長度: ${textContent.length} 字元`);
    return this._generateTitleAndDescription(textContent, segment);
  }

  async _generateTitleAndDescription(textContent, segment = null) {
    try {
      ...
      // 備用方案：使用改進的智能模板
      const fallbackContent = this.generateEnhancedFallback(textContent);
      return {
        ...fallbackContent,
...
👉 從陪伴到家務，2026 年可能是家用機器人元年！`;
  ...
  generateSmartFallback() {
    return {
      titles: this.getFallbackTitles(),
      description: this.getFallbackDescription(),
      tags: ['AI', 'AI懶人報', 'AI懶人報 Podcast', 'podcast', 'AI工具', '人工智慧']
    };
  }
...
👉 智能建議、程式碼解釋、測試生成，團隊協作更順暢！`;
  ...
  generateEnhancedFallback(text) {
    ...
    const tags = ['AI', 'AI懶人報', 'AI懶人報 Podcast', 'podcast', 'AI工具', '人工智慧', 'AI新聞', 'ChatGPT', 'Claude', 'OpenAI', 'Gemini', 'AI教學', 'AI應用', '科技新聞'];
...
`;
    ...
    const toolDescriptions = [
      { name: tools[0] || 'ChatGPT', feature: '程式碼生成再進化，寫 App 像寫作文', value: '全新程式模式支援多語言開發，初學者也能 30 分鐘做出原型！' },
      { name: tools[1] || 'Claude', feature: 'AI 寫程式的天花板，Bug 偵測神準', value: '上傳截圖自動生成前端代碼，設計稿秒變真實網頁！' },
      { name: tools[2] || 'Cursor', feature: 'AI 編程助手內建，寫程式效率翻 10 倍', value: '智能補全、自動重構、Bug 修復，連資深工程師都在用！' },
      { name: tools[3] || 'GitHub Copilot', feature: '微軟 AI 程式夥伴，開發者必備神器', value: '智能建議、程式碼解釋、測試生成，團隊協作更順暢！' },
      { name: tools[4] || 'Replit', feature: '零基礎做 App 的最佳選擇', value: '描述需求就能生成完整專案，部署上線一鍵搞定！' }
    ];
